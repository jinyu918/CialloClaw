...
- 回到 `docs/work-priority-plan.md` 检查本次命中的任务项是否已经满足勾选标准
- 若本次代码 / 测试 / 文档已经满足对应任务项，必须在同一次工作中同步打勾，不得留到后续任务补记
...
- 每次新增或调整正式协议方法、正式字段、正式通知、正式状态、核心表结构、排期勾选或主链路语义时，都必须在同一次工作中同步回写对应真源文档
- 每次完成 `docs/work-priority-plan.md` 中的现有任务项时，都必须在同一次工作中同步更新对应勾选状态

禁止：

- 在当前改动范围内保留中文注释或失效注释
- 以“后面再补注释”为理由留尾债
- 以“代码已经改完，文档之后再补”为理由把真源更新拆到未来任务
- 已经满足分工文档勾选标准却不回写 `docs/work-priority-plan.md`
